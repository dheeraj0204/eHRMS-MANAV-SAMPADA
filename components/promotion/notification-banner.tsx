  ...
  return isVisible ? (
    <>
      <div className="relative mb-8 overflow-hidden rounded-3xl">
        ...
        <div className="absolute inset-0 bg-gradient-to-r from-blue-600 via-cyan-500 to-teal-600 opacity-90" />
        ...
        <div className="relative z-10 px-8 py-6 text-white">
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h3 className="text-lg font-bold text-white">🎉 Promotion Alert!</h3>
              <p className="text-sm text-white/80">Next Stage Promotion in Pipeline</p>
            </div>
          </div>

          {/* Employee Info */}
          <div className="bg-white/10 backdrop-blur-md rounded-2xl p-5 mb-6 border border-white/20">
            <p className="text-sm text-white/80 mb-2">Promoted Employee</p>
            <div className="flex items-baseline justify-between mb-3">
              <h4 className="text-2xl font-bold text-white">{promotionData.employee}</h4>
              <span className="text-xs bg-white/20 px-3 py-1 rounded-full text-white">{promotionData.department}</span>
            </div>
            <div className="flex items-center gap-2 text-sm text-white/80">
              <span>{promotionData.currentDesignation}</span>
              <span className="text-white">→</span>
              <span className="font-semibold text-white">{promotionData.newDesignation}</span>
            </div>
          </div>
          ...
          <div className="mb-6">
            <div className="bg-white/10 backdrop-blur-md rounded-2xl p-5 border border-white/20">
              <p className="text-sm text-white/80 mb-3 uppercase tracking-wider font-semibold">Salary Increment</p>
              <div className="grid grid-cols-3 gap-4 text-center">
                ...
                <div>
                  <p className="text-xs text-white/60 mb-1">New Salary</p>
              ...
              <div className="mt-4 pt-4 border-t border-white/20 text-center">
                <p className="text-sm text-white/80">Salary Increase</p>
          ...
          {/* Benefits Grid */}
          <div>
            <p className="text-sm text-white/80 mb-3 uppercase tracking-wider font-semibold">Additional Benefits</p>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {promotionData.benefits.map((benefit, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className="bg-white/10 backdrop-blur-md rounded-xl p-4 border border-white/20 hover:bg-white/20 transition-all duration-300 text-center transform hover:scale-105"
                  >
                    <Icon className={`w-5 h-5 mx-auto mb-2 ${benefit.color}`} />
                    <p className="text-xs text-white/70 mb-2">{benefit.label}</p>
                    <p className="text-sm font-bold text-white">{benefit.value}</p>
                  </div>
                )
              })}
            </div>
          ...
          <div className="mt-6 flex gap-3 justify-center">
            <button
              ...
              className="px-6 py-2 bg-white/20 hover:bg-white/30 backdrop-blur-md rounded-xl font-semibold text-white transition-all duration-200 border border-white/30 disabled:opacity-50 disabled:cursor-not-allowed"
            >
            ...
            <button
              ...
              disabled={hasResponded}
              className="px-6 py-2 bg-white text-blue-600 hover:bg-white/90 rounded-xl font-semibold transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
            >
          ...
          {hasResponded && (
            <div className="mt-4 text-center">
              {response === 'accepted' ? (
                <p className="text-sm text-green-300 font-semibold">✓ Promotion Accepted Successfully!</p>
              ) : (
                <p className="text-sm text-amber-300 font-semibold">Your response has been recorded</p>
              )}
            </div>
          )}
        </div>
